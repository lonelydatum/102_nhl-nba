...
function start(barOptions) {
  ...
  if (universalBanner.size === "300x250" || universalBanner.size === "160x600" || universalBanner.size === "300x600") {
    ...
    tl.from(".t3", stag(vh));
    tl.to([".logos", ".t3"], { duration: 0.3, opacity: 0 }, "+=" + READ.t3);
    tl.from(".t4", { duration: 0.3, opacity: 0 });
  }
...
exports.animate_bars_vertical = animate_bars_vertical;
exports.stag = stag;
...
function start(barOptions) {
  var vh = arguments.length <= 1 || arguments[1] === undefined ? { x: -_commonJsHockeyJs.size.w } : arguments[1];

  var tl = (0, _commonJsHockeyJs.init)();

  var fun = barOptions.HEIGHT > barOptions.WIDTH ? _commonJsHockeyJs.animate_bars_horizontal : _commonJsHockeyJs.animate_bars_vertical;
  fun(barOptions);
  TweenLite.to(".hero img", _extends({ duration: 3 }, barOptions.scale));

  tl.from(".t1", (0, _commonJsHockeyJs.stag)(vh), "+=.4");
  tl.to(".t1", { duration: 0.3, opacity: 0 }, "+=" + _commonJsHockeyJs.READ.t1);

  tl.from(".t2", (0, _commonJsHockeyJs.stag)(vh));
  var listter = [".frame1", ".logos"];

  tl.to(listter, { duration: 0.3, opacity: 0 }, "+=" + _commonJsHockeyJs.READ.t2);

  tl.to([".frame2"], { duration: 0.3, opacity: 1 }, "t2");

  tl.from(".t3", (0, _commonJsHockeyJs.stag)(vh));
  tl.from(".t4", { duration: 0.3, opacity: 0 });

  tl.from([".cta", ".legalBtn"], { duration: 0.3, opacity: 0 });

  tl.add((0, _commonJsHockeyJs.olg)());
}
...
start(barOptions, { y: _commonJsHockeyJs.size.h });
